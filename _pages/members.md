---
layout: archive
title: "Members"
permalink: /members/
author_profile: false
---

<style>
.member-grid {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(220px, 1fr));
  gap: 25px;
  margin-top: 20px;
  margin-bottom: 40px;
}
.member-card {
  text-align: center;
  background-color: var(--global-bg-color);
  border: 1px solid var(--border-color, #eaeaea);
  border-radius: 12px;
  padding: 20px;
  box-shadow: 0 4px 6px rgba(0,0,0,0.05);
  transition: transform 0.2s ease, box-shadow 0.2s ease;
}
.member-card:hover {
  transform: translateY(-5px);
  box-shadow: 0 10px 20px rgba(0,0,0,0.1);
}
.member-photo {
  width: 140px;
  height: 180px;
  object-fit: cover;
  border-radius: 5px;
  margin-bottom: 15px;
}
.member-name {
  font-weight: bold;
  font-size: 1.1em;
  margin-bottom: 5px;
  line-height: 1.3;
}
.member-title {
  font-size: 0.9em;
  color: var(--global-text-color-light, #555);
  margin-bottom: 8px;
}
.member-email {
  font-size: 0.85em;
  word-break: break-all;
}
</style>

## Principal Investigator
<div class="member-grid">
  <div class="member-card">
    <img src="/images/members/member_01.jpeg" class="member-photo" alt="Soochahn Lee">
    <div class="member-name">이수찬<br>Soochahn Lee</div>
    <div class="member-title">Associate Professor</div>
    <div class="member-email">
      <a href="mailto:sclee@kookmin.ac.kr" title="sclee@kookmin.ac.kr"><i class="fa fa-envelope fa-lg" aria-hidden="true"></i></a>&nbsp;&nbsp;
      <a href="https://scholar.google.com/citations?user=zG_owA8AAAAJ" title="Google Scholar" target="_blank" rel="noopener noreferrer"><i class="ai ai-google-scholar fa-lg"></i></a>&nbsp;&nbsp;
      <a href="https://orcid.org/0000-0002-2975-2519" title="ORCID" target="_blank" rel="noopener noreferrer"><i class="ai ai-orcid fa-lg"></i></a>
    </div>
  </div>
</div>

## Graduate Students
<div class="member-grid">
  <div class="member-card">
    <img src="/images/members/member_02.png" class="member-photo" alt="안수빈 Subin An">
    <div class="member-name">안수빈<br>Subin An</div>
    <div class="member-title">Ph.D.</div>
    <div class="member-email"><a href="mailto:nesquiq@kookmin.ac.kr" title="nesquiq@kookmin.ac.kr"><i class="fa fa-envelope fa-lg" aria-hidden="true"></i></a></div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_03.jpg" class="member-photo" alt="이정익 JeongIk Lee">
    <div class="member-name">이정익<br>JeongIk Lee</div>
    <div class="member-title">M.S.</div>
    <div class="member-email"><a href="mailto:20161699@kookmin.ac.kr" title="20161699@kookmin.ac.kr"><i class="fa fa-envelope fa-lg" aria-hidden="true"></i></a></div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_04.jpg" class="member-photo" alt="정승은 SeungEun Chung">
    <div class="member-name">정승은<br>SeungEun Chung</div>
    <div class="member-title">M.S.</div>
    <div class="member-email"><a href="mailto:agjung0222@kookmin.ac.kr" title="agjung0222@kookmin.ac.kr"><i class="fa fa-envelope fa-lg" aria-hidden="true"></i></a></div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_05.jpeg" class="member-photo" alt="반요한 Yohan Ban">
    <div class="member-name">반요한<br>Yohan Ban</div>
    <div class="member-title">M.S.</div>
    <div class="member-email"><a href="mailto:yohan1312@kookmin.ac.kr" title="yohan1312@kookmin.ac.kr"><i class="fa fa-envelope fa-lg" aria-hidden="true"></i></a></div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_06.png" class="member-photo" alt="나이형 Samuel Low Yu Hang">
    <div class="member-name">나이형<br>Samuel Low Yu Hang</div>
    <div class="member-title">M.S.</div>
    <div class="member-email"><a href="mailto:lysamuel-2001@kookmin.ac.kr" title="lysamuel-2001@kookmin.ac.kr"><i class="fa fa-envelope fa-lg" aria-hidden="true"></i></a></div>
  </div>
</div>

## Alumni
<div class="member-grid">
  <div class="member-card">
    <img src="/images/members/member_07.jpeg" class="member-photo" alt="맹주완 Juwan Maeng">
    <div class="member-name">맹주완<br>Juwan Maeng</div>
    <div class="member-title">M.S.</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_08.jpg" class="member-photo" alt="이건 Gun Lee">
    <div class="member-name">이건<br>Gun Lee</div>
    <div class="member-title">M.S.</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_09.jpeg" class="member-photo" alt="채재민 JaeMin Chae">
    <div class="member-name">채재민<br>JaeMin Chae</div>
    <div class="member-title">M.S.<br>SNUBH</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_10.jpg" class="member-photo" alt="고소정 Sojeong Go">
    <div class="member-name">고소정<br>Sojeong Go</div>
    <div class="member-title">M.S.<br>SNUBH</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_11.jpg" class="member-photo" alt="김의찬 UiChan Kim">
    <div class="member-name">김의찬<br>UiChan Kim</div>
    <div class="member-title">M.S.</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_12.jpeg" class="member-photo" alt="임정선 Jungsun Im">
    <div class="member-name">임정선<br>Jungsun Im</div>
    <div class="member-title">M.S.<br>LG Electronics</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_13.jpg" class="member-photo" alt="이종진 Jongjin Lee">
    <div class="member-name">이종진<br>Jongjin Lee</div>
    <div class="member-title">M.S.</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_14.jpg" class="member-photo" alt="이다슬 Daseul Lee">
    <div class="member-name">이다슬<br>Daseul Lee</div>
    <div class="member-title">M.S.<br>NH</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_15.jpg" class="member-photo" alt="아말 Amal Saleem">
    <div class="member-name">아말<br>Amal Saleem</div>
    <div class="member-title">M.S.</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_16.jpeg" class="member-photo" alt="전효기 Hyogi Jeon">
    <div class="member-name">전효기<br>Hyogi Jeon</div>
    <div class="member-title">M.S.<br>LG Electronics</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_17.jpg" class="member-photo" alt="정철희 Jeongcheol Jeong">
    <div class="member-name">정철희<br>Jeongcheol Jeong</div>
    <div class="member-title">M.S.</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_18.jpg" class="member-photo" alt="노경진 Kyungjin Noh">
    <div class="member-name">노경진<br>Kyungjin Noh</div>
    <div class="member-title">M.S.<br>StradVision</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_19.jpg" class="member-photo" alt="임종수 JongSu Im">
    <div class="member-name">임종수<br>JongSu Im</div>
    <div class="member-title">M.S.</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_20.jpeg" class="member-photo" alt="지영훈 Younghoon Ji">
    <div class="member-name">지영훈<br>Younghoon Ji</div>
    <div class="member-title">M.S.<br>VUNO</div>
  </div>
  <div class="member-card">
    <img src="/images/members/member_21.jpg" class="member-photo" alt="김주영 Jooyoung Kim">
    <div class="member-name">김주영<br>Jooyoung Kim</div>
    <div class="member-title">M.S.<br>SNUBH</div>
  </div>
</div>

## Undergraduate Research Associate Alumni

* **2025:** 이정익, 이준혁, 권민지
* **2024:** 박현성, 경세진, 반요한, 박가현, 김준엽, 유민아, 한나래
* **2023:** 이재형, 김태규, 이현수, 박민영
* **2022:** 김의찬, 조상현, 고병오, 강소영, 이기정, 임종수, 황순규, 맹주완, 정승은, 이승은, 조예진, 김수민, 천연진, 남주호, 주용곤
* **2020:** 이 건, 조효원, 박민제, 정지원, 한병근
* **2019:** 양재원
* **2018:** 권광훈, 조재민, 연태민, 이준호, 반원규, 김남호, 임경민, 심규찬
* **2017:** 손종효, 김찬희, 정영진, 박세진, 임경규, 임충기, 한민재, 전해원, 코스마스
* **2016:** 최대광, 김상헌, 전창환, 황무현, 이유현, 백인규, 임태선, 김용익
* **2015:** 고소정, 이다슬, 김희경, 박성기, 김동기, 조연, 김동환, 김민용, 김윤주
* **2014:** 이강찬, 노경진, 남진우, 김태균, 한상유, 우관희, 정성수, 유상동, 임종은, 이다솔, 정규상, 박경주, 유재석, 전지혜, 박세연
